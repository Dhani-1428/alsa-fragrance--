...
import { getPendingOrders, confirmOrder, getOrderById, getOrderByOrderNumber, getOrderByEmailAndStatus } from "@/lib/orders-mysql"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (orderId) {
      ...
      const order = await getOrderByEmailAndStatus(email, "pending")
      if (order) {
        orderToConfirm = await confirmOrder(order.id)
      }
    } else if (phoneNumber) {
